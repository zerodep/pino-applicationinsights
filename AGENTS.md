...
- `npm test` — runs the full pipeline: `npm run build` (pretest) → mocha against `test/src/**/*.js` and `test/commonjs/**/*.cjs` → `npm run lint` → `npm run tsc` → `npm run test:md` (posttest). **Lint must always run after tests** — never skip it, never reorder it. Don't paper over an SDK warning with an env var; fix the underlying nock setup.
- `npm run lint` — `eslint . --cache && prettier . --check --cache`.
- `npm run tsc` — `tsc -p test`: type-checks every test file (`test/**/*`, JS included via `checkJs`) plus `src/` and the example app against the published `types/index.d.ts`. `test/globals.d.ts` declares the `expect` global registered by `chai/register-expect.js`. Tests that deliberately pass bad input use `// @ts-expect-error` on the offending line; everything else is fixed with JSDoc annotations, never by loosening `test/tsconfig.json`.
...
- Run a single test: `npx mocha test/src/compose-test.js` (or `--grep "pattern"`).
...
- `test/typescript/consumer.ts` is a compile-only fixture: real consumer code written against the published `.d.ts` surface, checked by `npm run tsc` and never executed. Treat a type error there as a public API regression.
...
- The example app reads `example/config/<NODE_ENV>.json` and `example/.env` through `exp-config@5`, which requires `NODE_ENV` to be set explicitly and resolves the config directory from `CONFIG_BASE_PATH` (default: cwd). `test/helpers/setup.js` sets both so mocha can run from the repo root; `npm start -w example` runs with cwd `example/`, and its `start` script sets `NODE_ENV`.
